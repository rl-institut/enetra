...
Could be slightly rewritten to make it work with any app.
Right now database constraints must be handled manually before generating the instances,
e.g. handle unique values
"""
...
class Visitor(Protocol):
    def visit(self, queryset: QuerySet) -> None: ...


def visit_all_scenario_queries(visitor: Visitor, scenario: Scenario):
    """
    Iterates over all models in the "ports" Django app and passes scenario-related
    querysets to a visitor.

    For each model, this function attempts to build a queryset filtered by the
    provided `scenario` via a `scenario` foreign key and sends it to
    `visitor.visit()`. Models that do not define a `scenario` field are skipped.

    The `Scenario` model itself is handled specially: instead of following any
    parent relationship, the function constructs a queryset containing only the
    provided scenario instance (`id=scenario.id`) and passes that to the visitor.

    Args:
        visitor: An object exposing a `visit(queryset)` method that processes
            querysets.
        scenario (Scenario): The scenario instance used to filter model records.

    Notes:
        - Only models within the "ports" app are inspected.
        - Models without a `scenario` field are ignored.
    """
...
class ScenarioJSONImporterExporter:
    ...
    def visit(self, elements: QuerySet) -> None:
        """Visit each element of the query set and store the serialized data"""
    ...
    def load(self, path: Path):
        """Load and parse data from a filepath.
        Deletes existing object_data"""
        with open(path, "rb") as f:
            self.loads(f.read())

    def generate_instances(self):
        """Use the stored object data to generate instances
        The constructors are found using the model_name which is the key of the data dict
        """
    ...
    def create_creation_order(self) -> list[str]:
        """Return a list of models in the order they should be created"""
    ...
    def create_many_to_many(self):
        """Use the generated objects and link them in the many to many tables"""
